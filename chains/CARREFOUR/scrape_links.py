#!/usr/bin/env python3
"""Scrape CARREFOUR transparency index pages and build a normalized links map."""
...
EMBEDDED_PATH_RE = re.compile(r"const\s+path\s*=\s*['\"](?P<path>[^'\"]+)['\"]")
EMBEDDED_FILES_RE = re.compile(r"const\s+files\s*=\s*(?P<files>\[[\s\S]*?\])\s*;")
...
def parse_updated_at(raw_value: str) -> Optional[str]:
    if not raw_value:
        return None
    for fmt in ("%m/%d/%Y %I:%M:%S %p", "%m/%d/%Y %H:%M:%S", "%H:%M %d-%m-%Y"):
        try:
...
def extract_embedded_rows(html: str, base_url: str, logger: Optional[logging.Logger] = None) -> List[RawRow]:
    path_match = EMBEDDED_PATH_RE.search(html)
    files_match = EMBEDDED_FILES_RE.search(html)
    if not path_match or not files_match:
        return []

    files_blob = files_match.group("files")
    try:
        files_data = json.loads(files_blob)
    except json.JSONDecodeError as error:
        if logger:
            logger.warning("Failed parsing embedded files JSON: %s", error)
        return []

    path_part = path_match.group("path").strip("/")
    extracted_rows: List[RawRow] = []
    for index, item in enumerate(files_data, start=1):
        ...
        if not raw_name:
            continue
        file_name = raw_name[4:] if raw_name.startswith("NULL") else raw_name

        modified = str(item.get("modified") or "").strip()
        size = str(item.get("size") or "").strip()
        suffix = Path(file_name).suffix.lower()
        file_ext = suffix[1:] if suffix.startswith(".") else suffix
        category_match = re.match(r"^(?P<prefix>[A-Za-z]+)", file_name)
        category = (category_match.group("prefix") if category_match else "unknown").lower()
        download_url = urljoin(base_url, f"{path_part}/{file_name}")
...
def normalize_row(row: RawRow) -> Dict[str, object]:
    ...
    if file_prefix and not file_type:
...
def scrape_pages(
    ...
    if not raw_rows:
        embedded_rows = extract_embedded_rows(first_html, BASE_URL, logger=logger)
        if embedded_rows:
            if logger:
                logger.info("Falling back to embedded JS extraction, found %s files", len(embedded_rows))
            raw_rows = embedded_rows

    if target_max_page >= 2 and parser.rows:
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Scrape CARREFOUR file links map")
