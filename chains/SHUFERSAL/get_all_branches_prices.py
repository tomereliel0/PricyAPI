#!/usr/bin/env python3
"""Fetch prices for all SHUFERSAL branches using get_branch_prices.py."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Fetch prices for all SHUFERSAL branches")
...
def main() -> int:
    ...
    try:
        ...
        logger.info(
            "Completed %s/%s branches, success=%s, failed=%s. Summary: %s",
            len(results),
            len(store_ids),
            len(successes),
            len(failures),
