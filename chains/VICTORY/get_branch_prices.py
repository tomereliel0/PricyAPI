#!/usr/bin/env python3
"""Fetch and normalize VICTORY prices for a single store_id."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Fetch and normalize VICTORY branch prices")
    ...
    parser.add_argument("--output", default=None, help="Output JSON path (default: chains/VICTORY/prices/{store_id}.json)")
...
def parse_xml_prices(xml_text: str, fallback_store_id: str) -> Tuple[Dict[str, Optional[str]], List[Dict[str, object]], int]:
    ...
    if not item_nodes:
        item_nodes = list(iter_by_local_name(root, "Product"))
    ...
    for item in item_nodes:
        ...
        record = {
            ...
            "manufacturer_name": clean_nullable(item_map.get("manufacturername") or item_map.get("manufacturename")),
            "manufacturer_item_description": clean_nullable(
                item_map.get("manufactureritemdescription") or item_map.get("manufactureitemdescription")
            ),
