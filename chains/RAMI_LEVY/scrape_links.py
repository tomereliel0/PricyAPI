#!/usr/bin/env python3
"""Scrape RAMI_LEVY Cerberus file listings and build links-map.json."""
...
FILE_NAME_RE = re.compile(
    r"^(?P<prefix>[A-Za-z]+)(?P<chain_id>\d+)-(?P<branch_id>\d{3})-(?P<ts>\d{8}(?:-\d{6})?|\d{12})(?:\.(?P<ext>[A-Za-z0-9]+))?$"
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Scrape RAMI_LEVY file links map")
