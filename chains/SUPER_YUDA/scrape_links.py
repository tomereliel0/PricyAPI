#!/usr/bin/env python3
"""Scrape SUPER_YUDA Cerberus file listings and build links-map.json."""
...
FILES_URL = "https://publishedprices.co.il/file"
FILES_FOLDER_URL = "https://publishedprices.co.il/file/d/Yuda/"
YUDA_DIR_CD = "/Yuda"
USERNAME = "yuda_ho"
PASSWORD = "Yud@147"
USER_AGENT = "PricyAPI/0.1 (+https://github.com/)"
FILE_JSON_DIR_URL = "https://publishedprices.co.il/file/json/dir"
...
class FilesPageParser(HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs) -> None:  # type: ignore[override]
        ...
        if normalized_path in {"/file", "/file/d/Yuda"}:
...
def extract_file_name(download_url: str) -> Optional[str]:
    ...
    if len(parts) < 3 or parts[0] != "file" or parts[1] != "d":
        return None
...
def normalize_link(download_url: str, page_url: str) -> Optional[Dict[str, object]]:
    ...
    if not file_name:
        return None
    if file_name.lower() in {"yuda", "stores"}:
        return None
    if "." not in file_name and FILE_NAME_RE.match(file_name) is None:
        return None
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Scrape SUPER_YUDA file links map")
